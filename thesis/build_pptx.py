...
NOTES = {
    ...
    "goal": (
        ...
        "ймовірних збігів. Ефективність я міряю формальними метриками — часткою "
        "влучень істинного збігу в перші позиції переліку (Hit@K) і середнім оберненим "
        "рангом MRR, а також часом формування переліку, і досягнення мети підтверджую "
        "експериментом. Щоб "
    ...
    "subject": (
        "Об'єкт роботи — це процес розроблення програмного забезпечення для "
        "структурованого подання та автоматичного зіставлення оголошень про загублених "
        "і знайдених тварин, тобто інженерна задача зі спеціальності 121, а не сам "
    ...
    "arch": (
        "Архітектурно застосунок — багаторівневий: рівень подання (клієнт на React), "
        "рівень застосунку (сервер на NestJS) і рівень даних (PostgreSQL та файлове "
        "сховище). Це не дволанкова клієнт-серверна схема — доступ до даних винесено "
        "в окремий рівень. Сервер побудовано за принципом портів та адаптерів, тож "
        "модуль зв'язків не лізе напряму в таблицю оголошень, а працює через її порт — "
        "межі модулів чіткі."
    ),
    "db": (
